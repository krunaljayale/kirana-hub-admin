...
const initialProducts: Product[] = [
  ...
  { 
    ...
    stock: 8, 
    unit: "1L", 
    images: ["https://cdn.grofers.com/cdn-cgi/image/f=auto,fit=scale-down,q=70,metadata=none,w=1080/da/cms-assets/cms/product/61c67eb1-3f1b-423c-bc55-6b7fba83a5c9.png"] 
  },
  { 
    id: 3, 
    name: "Tata Salt", 
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      <button
        ...
        className={`w-full flex items-center justify-between px-4 py-3 text-left rounded-xl border transition-all ${
          isOpen 
        ...
        } dark:text-white text-sm font-medium focus:outline-none`}
      >
    ...
    <div className="relative bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-full">
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-200">
          ...
          <div className="bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-w-2xl rounded-none sm:rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden flex flex-col max-h-[100dvh] sm:max-h-[90vh] animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  <div className="col-span-1 md:col-span-2">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 dark:text-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400"
            ...
            <div className="p-4 sm:p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex gap-4 z-50 relative shrink-0">
              ...
              <button form="productForm" type="submit" className="flex-1 px-4 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold shadow-lg shadow-indigo-500/30 transition-all hover:scale-[1.02]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm p-6 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 scale-100 animate-in zoom-in-95 duration-200">
              ...
              <p className="text-center text-slate-500 dark:text-slate-400 text-sm mb-8">
                Are you sure? This will remove the item from your inventory permanently.
              </p>
              <div className="flex gap-3">
                <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  Cancel
                </button>
                <button onClick={handleDelete} className="flex-1 px-4 py-3 rounded-xl bg-red-500 text-white font-bold shadow-lg shadow-red-500/30 hover:bg-red-600 transition-all">
                  Delete
                </button>
              </div>
           </div>
      ...
      <div className="p-5 flex flex-col xl:flex-row gap-4 justify-between items-center border-b border-slate-100 dark:border-slate-800/50 bg-white dark:bg-slate-900 shrink-0">
        ...
        <button 
          ...
          className="w-full xl:w-auto flex items-center justify-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold shadow-lg shadow-indigo-500/30 transition-all hover:scale-105 active:scale-95"
        >
                ...
                <div key={product.id} className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
                  ...
                  <div className="flex gap-4">
                     <div className="relative h-20 w-20 flex-shrink-0 rounded-xl bg-slate-100 dark:bg-slate-700 overflow-hidden border border-slate-200 dark:border-slate-600">
                        {product.images.length > 0 ? (
                           ...
                           <div className="h-full w-full flex items-center justify-center text-slate-400 font-bold bg-slate-50 dark:bg-slate-800">
                              {product.name.charAt(0)}
                           </div>
                        )}
                        {/* Status Badge Over Image */}
                        {product.stock < 10 && (
                           <div className="absolute top-0 right-0 p-1 bg-red-500 text-white rounded-bl-lg shadow-sm">
                     ...
                     <div className="flex-1">
                        <div className="flex justify-between items-start">
                           ...
                           <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide border ${status.color}`}>
                             {status.label}
                           </span>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                     <button 
                       ...
                       className="flex items-center justify-center gap-2 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors"
                     >
                     ...
                     <button 
                       ...
                       className="flex items-center justify-center gap-2 py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-100 dark:bg-red-900/20 dark:border-red-900/30 dark:text-red-400 font-bold text-sm hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors"
                     >
                       <TrashIcon className="h-4 w-4" />
                       Delete
                     </button>
                  ...
                  return (
                    <tr key={product.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="relative h-12 w-12 rounded-xl bg-slate-100 dark:bg-slate-700 overflow-hidden border border-slate-200 dark:border-slate-600 flex-shrink-0 shadow-sm">
                            ...
                            {product.images.length > 1 && (
                              <div className="absolute bottom-0 right-0 bg-black/70 backdrop-blur-sm text-white text-[9px] px-1.5 py-0.5 font-bold rounded-tl-md">
                                +{product.images.length - 1}
                              </div>
                            )}
                          </div>
                      ...
                      <td className="p-4 text-sm font-medium text-slate-600 dark:text-slate-400">
                        <span className="inline-block px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs">
                          {product.category}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                           <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold uppercase border ${status.color}`}>
                             {status.label}
                           </span>
